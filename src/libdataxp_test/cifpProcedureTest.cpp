...
namespace
{
    ...
    static string sampleLecoApproachCifp()
    {
        return
...
TEST(XPCifpReaderTest, parsesApproachAltitudeAndSpeedConstraints)
{
    auto host = make_shared<ProcedureTestHostServices>();
    XPCifpReader reader(host);
    ...
    ASSERT_FALSE(selection.procedureTrack.waypoints.empty());

    const auto co401 = find_if(selection.procedureTrack.waypoints.begin(), selection.procedureTrack.waypoints.end(), [](const XPCifpReader::WaypointWithLocation& waypoint) {
    ...
    const auto co403 = find_if(selection.missedTrack.waypoints.begin(), selection.missedTrack.waypoints.end(), [](const XPCifpReader::WaypointWithLocation& waypoint) {
        return waypoint.name == "CO403";
    });
    ASSERT_NE(co403, selection.missedTrack.waypoints.end());
    EXPECT_FLOAT_EQ(co403->altitudeConstraint, 4000.0f);
    EXPECT_EQ(co403->altitudeConstraintType, '+');
    EXPECT_FLOAT_EQ(co403->speedConstraint, 200.0f);
...
TEST(FlightPlanTest, rebuildsTerrainAwareProcedureAltitudes)
{
    ...
    EXPECT_GE(starTargetAltitudes.front(), 5000.0f);
