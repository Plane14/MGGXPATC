...
shared_ptr<Airport> createDepartureHoldShortTestAirport()
{
    auto host = TestHostServices::create();

    auto n_10_10 = shared_ptr<TaxiNode>(new TaxiNode(3010, UniPoint::fromGeo(host, {30.10, 45.10})));
    auto n_10_20 = shared_ptr<TaxiNode>(new TaxiNode(3020, UniPoint::fromGeo(host, {30.10, 45.20})));
    auto n_10_30 = shared_ptr<TaxiNode>(new TaxiNode(3030, UniPoint::fromGeo(host, {30.10, 45.30})));
    auto n_20_10 = shared_ptr<TaxiNode>(new TaxiNode(4010, UniPoint::fromGeo(host, {30.20, 45.10})));
    auto n_30_10 = shared_ptr<TaxiNode>(new TaxiNode(5010, UniPoint::fromGeo(host, {30.30, 45.10})));
    n_30_10->setRouteStart(true);

    auto e_3010_3020 = shared_ptr<TaxiEdge>(new TaxiEdge(30103020, "09/27", 3010, 3020, TaxiEdge::Type::Runway));
    auto e_3020_3030 = shared_ptr<TaxiEdge>(new TaxiEdge(30203030, "09/27", 3020, 3030, TaxiEdge::Type::Runway));
    auto e_4010_3010 = shared_ptr<TaxiEdge>(new TaxiEdge(40103010, "HS1", 4010, 3010, TaxiEdge::Type::HoldShort));
    auto e_5010_4010 = shared_ptr<TaxiEdge>(new TaxiEdge(50104010, "A", 5010, 4010, TaxiEdge::Type::Taxiway));

    auto rwy_0927 = shared_ptr<Runway>(new Runway(
        Runway::End("09", 0, 0, n_10_10->location()),
        Runway::End("27", 0, 0, n_10_30->location()),
        30
    ));

    return WorldBuilder::assembleAirport(
        host,
        testHeader,
        { rwy_0927 },
        {},
        { n_10_10, n_10_20, n_10_30, n_20_10, n_30_10 },
        { e_3010_3020, e_3020_3030, e_4010_3010, e_5010_4010 }
    );
}
...
TEST(TaxiPathTest, departurePathIncludesRunwayEntryFromHoldShort)
{
    auto airport = createDepartureHoldShortTestAirport();
    auto startNode = airport->taxiNet()->getNodeById(5010);

    auto path = airport->taxiNet()->tryFindDepartureTaxiPathToRunway(
        startNode->location().geo(),
        airport->getRunwayEndOrThrow("09"));

    ASSERT_TRUE(!!path);
    ASSERT_EQ(path->edges.size(), 2u);
    EXPECT_EQ(path->edges[0]->id(), 50104010);
    EXPECT_EQ(path->edges[1]->type(), TaxiEdge::Type::HoldShort);
    EXPECT_EQ(path->edges[1]->node1()->id(), 4010);
    EXPECT_EQ(path->edges[1]->node2()->id(), 3010);
    EXPECT_EQ(path->toNode->id(), 3010);
}
