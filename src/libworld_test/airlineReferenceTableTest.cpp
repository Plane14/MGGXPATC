...
TEST(AirlineReferenceTableTest, tryFindByName_success)
{
    AirlineReferenceTable::Entry entry;

    bool result = AirlineReferenceTable::tryFindByName("Vueling Airlines", entry);

    EXPECT_TRUE(result);
    EXPECT_EQ(entry.icao, "VLG");
    EXPECT_EQ(entry.callsign, "VUELING");
    EXPECT_EQ(entry.name, "Vueling Airlines");
    EXPECT_EQ(entry.regionIcao, "Spain");
}
