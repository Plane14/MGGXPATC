...
namespace world
{
    ...
    private:
        ...
        void buildVerbalizerMap()
        {
            ...
            MAP_VERBALIZER_INTENT(ControllerHandoffIntent, verbalizeControllerHandoff);
            MAP_VERBALIZER_INTENT(PilotCheckInWithRadarIntent, verbalizeCheckInWithRadar);
            MAP_VERBALIZER_INTENT(ControllerRadarContactIntent, verbalizeRadarContact);
...
//            }});
        ...
        void verbalizeControllerHandoff(UtteranceBuilder& builder, shared_ptr<ControllerHandoffIntent> intent)
        {
            builder.addData(spellCallsign(intent->subjectFlight()->callSign()));
            builder.addText("contact");
            if (intent->targetControl())
            {
                builder.addData(spellCallsign(intent->targetControl()->callSign()));
            }
            else
            {
                builder.addText("radar");
            }
            builder.addText("on");
            builder.addData(spellFrequency(intent->newFrequencyKhz()));
            builder.addFarewell("good day");
        }

        void verbalizeCheckInWithRadar(UtteranceBuilder& builder, shared_ptr<PilotCheckInWithRadarIntent> intent)
        {
            builder.addData(spellCallsign(intent->subjectControl()->callSign()));
            builder.addPunctuation();
            builder.addData(spellCallsign(intent->subjectFlight()->callSign()));
            builder.addText("with you at");
            builder.addData(spellAltitude(intent->altitudeFeet()));
        }

        void verbalizeRadarContact(UtteranceBuilder& builder, shared_ptr<ControllerRadarContactIntent> intent)
        {
            builder.addData(spellCallsign(intent->subjectFlight()->callSign()));
            builder.addPunctuation();
            builder.addText("radar contact");
            if (intent->subjectControl() && intent->subjectControl()->type() == ControllerPosition::Type::Approach)
            {
                builder.addText("continue approach");
            }
            else
            {
                builder.addText("continue as cleared");
            }
        }
...
//            }
        ...
        void verbalizeTakeoffClearance(UtteranceBuilder& builder, shared_ptr<TowerClearedForTakeoffIntent> intent)
        {
            ...
            if (weatherService)
            {
                auto weather = weatherService->getWeatherAt(runwayEnd.centerlinePoint().geo(), runwayEnd.elevationFeet() / FEET_IN_1_METER);
                const float windSpeedKt = weather.windSpeedMetersPerSecond * KNOT_IN_1_METER_PER_SEC;
                if (windSpeedKt < 2.5f)
                {
                ...
                else
                {
                    ...
                    if (weather.windGustMetersPerSecond > weather.windSpeedMetersPerSecond + 0.5f)
                    {
                        builder.addText("gust");
                        builder.addText(to_string((int)(weather.windGustMetersPerSecond * KNOT_IN_1_METER_PER_SEC + 0.5f)));
                    }
                }
            }
            else
        ...
        void verbalizeLandingClearance(UtteranceBuilder& builder, shared_ptr<TowerClearedForLandingIntent> intent)
        {
            ...
            if (weatherService)
            {
                ...
                else
                {
                    ...
                    builder.addText("cleared to land runway");
                }
            }
            else
            {
