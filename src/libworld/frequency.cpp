...
namespace world
{
    namespace
    {
        chrono::microseconds getConversationExpiryDelay(const shared_ptr<Transmission>& transmission)
        {
            ...
            if (transmission)
            {
                if (const auto utterance = transmission->verbalizedUtterance())
                {
                    istringstream words(utterance->plainText());
                    string word;
                    while (words >> word)
                    {
                        wordCount++;
            ...
            const int expirySeconds = max(8, min(18, 7 + static_cast<int>(wordCount / 3)));
            return chrono::seconds(expirySeconds);
        }
    }
...
//    };
    ...
    {
        auto completedTransmission = m_transmissionInProgress;
        logTransmission("DISPATCH COMPLETED TRANSMISSION", completedTransmission);

        auto intent = completedTransmission->intent();

        completedTransmission->m_endTimestamp = timestamp;
        completedTransmission->m_state = Transmission::State::Completed;
        ...
        {
            m_openConversationExpiryByIntentId.erase(intent->replyToId());
        }

        if (intent->conversationState() == Intent::ConversationState::Continue)
        {
            m_openConversationExpiryByIntentId[intent->id()] = timestamp + getConversationExpiryDelay(completedTransmission);
        }
    ...
    void Frequency::clearTransmissions()
    {
        ...
        m_criticalAwaiters.clear();
        m_lastTransmittedIntentId = 0;
        m_openConversationExpiryByIntentId.clear();
    }
    ...
    bool Frequency::wasSilentFor(chrono::milliseconds duration, uint64_t replyToId)
    {
        ...
        if (!m_openConversationExpiryByIntentId.empty())
        {
            return replyToId > 0 && m_openConversationExpiryByIntentId.find(replyToId) != m_openConversationExpiryByIntentId.end();
        }
...
//                    it->intent->isCritical() ? 1 : 0);
    ...
    void Frequency::checkConversationStateExpiry(chrono::microseconds timestamp)
    {
        ...
        vector<uint64_t> expiredIntentIds;
        for (const auto& pair : m_openConversationExpiryByIntentId)
        {
            if (timestamp >= pair.second)
            {
                expiredIntentIds.push_back(pair.first);
            }
        }

        for (const auto expiredIntentId : expiredIntentIds)
        {
            m_host->writeLog(
                "%d|WARNING: conversation state expired! state[%d] intent id[%d]",
                m_khz,
                Intent::ConversationState::Continue,
                expiredIntentId);
            m_openConversationExpiryByIntentId.erase(expiredIntentId);
        }
    }
}
