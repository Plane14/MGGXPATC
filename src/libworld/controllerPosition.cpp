...
namespace world
{
    ...
    void ControllerPosition::startListenOnFrequency()
    {
        m_frequency->addListener([=](shared_ptr<Intent> intent) {
            ...
            auto position = m_controller->position();
            if (!position)
            {
                return;
            }
            ...
            {
            case Intent::Direction::PilotToController:
                shouldReceive = (intent->subjectControl() == position);
                break;
            case Intent::Direction::ControllerToController:
                shouldReceive = (intent->subjectControl2() == position);
                break;
            default:
                shouldReceive = false;
                break;
