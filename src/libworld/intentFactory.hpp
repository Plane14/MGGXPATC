...
namespace world
{
    ...
    public:
        ...
        shared_ptr<Intent> pilotReportHoldingShort(shared_ptr<Flight> flight, shared_ptr<Airport> airport, const string& runway, const string& holdingPoint)
        {
            ...
            if (airport->header().icao() == flight->plan()->arrivalAirportIcao())
            {
                ground = m_helper.tryGetArrivalGround(flight, flight->aircraft()->location());
            }
            else
            {
                ground = m_helper.tryGetDepartureGround(flight);
            }
        ...
        {
            auto airport = flight && flight->phase() == Flight::Phase::Arrival
                ? m_helper.getArrivalAirport(flight)
                : m_helper.getDepartureAirport(flight);
            return pilotReportHoldingShort(flight, airport, runway, holdingPoint);
        }
        ...
        shared_ptr<Intent> controllerHandoff(
            shared_ptr<Flight> flight,
            shared_ptr<ControllerPosition> fromControl,
            shared_ptr<ControllerPosition> toControl,
            uint64_t replyToId = 0)
        {
            if (!flight || !fromControl || !toControl)
            {
                return nullptr;
            }

            return shared_ptr<Intent>(new ControllerHandoffIntent(
                m_nextIntentId++,
                replyToId,
                flight,
                fromControl,
                toControl,
                toControl->frequency()->khz()
            ));
        }

        shared_ptr<Intent> pilotCheckInWithRadar(shared_ptr<Flight> flight, shared_ptr<ControllerPosition> controller)
        {
            ...
            auto altitude = flight->aircraft()->altitude();
            float altitudeFeet = altitude.feet();
            if (altitude.type() == Altitude::Type::Ground)
            {
                altitudeFeet = m_host->queryTerrainElevationAt(flight->aircraft()->location());
            }
            ...
            return shared_ptr<Intent>(new PilotCheckInWithRadarIntent(
                m_nextIntentId++,
                flight,
                controller,
                static_cast<int>(altitudeFeet + 0.5f)
            ));
        }

        shared_ptr<Intent> controllerRadarContact(shared_ptr<Flight> flight, shared_ptr<ControllerPosition> controller, uint64_t replyToId)
        {
            if (!flight || !controller)
            {
                return nullptr;
            ...
            return shared_ptr<Intent>(new ControllerRadarContactIntent(
                m_nextIntentId++,
                replyToId,
                flight,
                controller
        ...
        shared_ptr<Intent> towerClearedForLanding(shared_ptr<LandingClearance> clearance, const vector<TrafficAdvisory>& traffic, uint64_t replyToId)
        {
            auto flight = clearance->header().issuedTo;
            auto landingPoint = m_helper.getLandingPoint(flight);
            auto ground = m_helper.tryGetArrivalGround(flight, landingPoint);
        ...
        {
            auto ground = m_helper.tryGetArrivalGround(flight, flight->aircraft()->location());
