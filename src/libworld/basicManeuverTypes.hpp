...
namespace world
{
    ...
    public:
        ...
        typedef function<void()> StopFunction;
    private:
        T m_startValue;
        T m_endValue;
        T m_lastValue;
    ...
    public:
        void progressTo(chrono::microseconds timestamp)
        {
            ...
            double progress = (m_duration.count() > 0)
                ? min(1.0, (double)elapsedAnimation.count() / (double)m_duration.count())
                : 1.0;
