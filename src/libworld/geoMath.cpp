...
namespace world
{
    ...
    void GeoMath::calculateTurn(const GeoMath::TurnData& input, GeoMath::TurnArc& output, shared_ptr<HostServices> host)
    {
        ...
        // Guard against a near-zero turn angle: tan(0) = 0 produces division by zero /
        // infinity which would corrupt all subsequent arc geometry.
        if (std::abs(angleBetweenEdges) < 0.001)
        {
            // Essentially a straight line – produce a degenerate (zero-length) arc.
            output.p0 = input.e1p1;
            output.p1 = input.e1p1;
            output.heading0 = static_cast<float>(GeoMath::radiansToDegrees(input.e1HeadingRad));
            output.heading1 = output.heading0;
            output.arcCenter = input.e1p1;
            output.arcRadius = 0.0;
            output.arcStartAngle = 0.0;
            output.arcEndAngle = 0.0;
            output.arcDeltaAngle = 0.0;
            output.arcLengthMeters = 0.0f;
            output.arcClockwise = true;
            if (host) host->writeLog("calculateTurn: near-straight angle %.6f rad, returning degenerate arc", angleBetweenEdges);
            return;
        }
