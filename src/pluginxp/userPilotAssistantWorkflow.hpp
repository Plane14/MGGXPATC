...
private:
    ...
    void build()
    {
        ...
        STATE(PilotState::HoldingShortRunway, [this]{
            auto reportHoldingShort = I.pilotReportHoldingShort(m_flight, m_departureAirport, m_holdingShortRunwayName, "");
            if (reportHoldingShort)
            {
                TRANSMIT(
                    reportHoldingShort->subjectControl()->frequency(),
                    reportHoldingShort);
            }
        }, {
        ...
        STATE(PilotState::CheckingInWithTower, [this]{
            if (m_holdingShortRunwayName.empty())
            {
