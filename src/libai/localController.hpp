...
namespace ai
{
    ...
    private:
        ...
        // Wind monitoring state for dynamic runway selection
        chrono::microseconds m_lastWindCheckTimestamp = chrono::microseconds(0);
        float m_lastMonitoredWindDirDegrees = -1.0f;
        static constexpr int64_t WIND_RECHECK_INTERVAL_US = 300000000LL; // 5 minutes
        static constexpr float WIND_CHANGE_THRESHOLD_DEG = 15.0f;
    ...
    public:
        ...
        void selectActiveRunways(vector<string>& departure, vector<string>& arrival) override
        {
            ...
            // Check if any existing runway mutex has active operations
            // If so, don't reselect runways to avoid dropping tracked aircraft
            for (const auto& mutexEntry : m_activeRunwayMutex)
            {
                if (mutexEntry.second && mutexEntry.second->hasActiveOperations())
                {
                    host()->writeLog(
                        "AICONT|Skipping runway reselection - active operations on runway[%s]",
                        mutexEntry.first.c_str());
                    // Return current active runways instead of selecting new ones
                    departure = airport->activeDepartureRunways();
                    arrival = airport->activeArrivalRunways();
                    return;
                }
            }

            // Check for airport-specific flow configuration based on wind (from apt.dat)
            auto weatherService = host()->services().tryGet<WeatherService>();
            if (weatherService)
            {
                auto airportLocation = airport->header().datum();
                ...
                if (flow)
                {
                    departure = flow->getDepartureRunways();
                    arrival = flow->getArrivalRunways();
                    
                    host()->writeLog(
                        "AICONT|Using apt.dat traffic flow[%s] for[%s] wind[%.0f°] departure[%zu] arrival[%zu]",
                        flow->name().c_str(),
            ...
            // Fall back to generic parallel runway selection
            if (airport->hasParallelRunways())
            {
                auto preferredArrivalRunwayName = airport->findPreferredArrivalRunway()->end1().name();
                ...
                if (longestGroup.size() >= 2)
                {
                    ...
                    departure.push_back(departureRunwayName);
                    arrival.push_back(preferredArrivalRunwayName);
                    if (longestGroup.size() > 2)
                    {
                ...
                {
                    arrival.push_back(preferredArrivalRunwayName);
                    departure.push_back(preferredArrivalRunwayName);
        ...
        void progressTo(chrono::microseconds timestamp) override
        {
            ...
            checkWindAndUpdateRunways(timestamp);
    ...
    private:
        ...
        void checkWindAndUpdateRunways(chrono::microseconds timestamp)
        {
            if ((timestamp - m_lastWindCheckTimestamp).count() < WIND_RECHECK_INTERVAL_US)
            {
                return;
            }
            m_lastWindCheckTimestamp = timestamp;
            ...
            const auto weather = weatherService->getWeatherAt(
                airport->header().datum(),
                airport->header().elevation() / FEET_IN_1_METER);
            const float newWindDir = weather.windDirectionTrueDegrees;

            // First sample: just record, no change yet
            if (m_lastMonitoredWindDirDegrees < 0.0f)
            {
                m_lastMonitoredWindDirDegrees = newWindDir;
                return;
            }

            const float windChange = abs(GeoMath::getTurnDegrees(m_lastMonitoredWindDirDegrees, newWindDir));
            if (windChange < WIND_CHANGE_THRESHOLD_DEG)
            {
                return;
            }

            // Do not change runways while aircraft are actively using them
            for (const auto& mutexEntry : m_activeRunwayMutex)
            {
                if (mutexEntry.second && mutexEntry.second->hasActiveOperations())
                {
                    host()->writeLog(
                        "AICONT|Wind shift %.0f\xc2\xb0\xe2\x86\x92%.0f\xc2\xb0 (%.0f\xc2\xb0) at [%s]: skipping - active ops on [%s]",
                        m_lastMonitoredWindDirDegrees, newWindDir, windChange,
                        airport->header().icao().c_str(),
                        mutexEntry.first.c_str());
                    m_lastMonitoredWindDirDegrees = newWindDir;
                    return;
            ...
            const auto newDep = flow->getDepartureRunways();
            const auto newArr = flow->getArrivalRunways();
            if (newDep == airport->activeDepartureRunways() && newArr == airport->activeArrivalRunways())
            {
                m_lastMonitoredWindDirDegrees = newWindDir;
                return;
            }
            ...
            host()->writeLog(
                "AICONT|Wind shift %.0f\xc2\xb0\xe2\x86\x92%.0f\xc2\xb0 (%.0f\xc2\xb0) at [%s]: reassigning to flow [%s]",
                m_lastMonitoredWindDirDegrees, newWindDir, windChange,
                airport->header().icao().c_str(),
                flow->name().c_str());
            m_lastMonitoredWindDirDegrees = newWindDir;

            // Apply new runways and rebuild mutexes
            airport->forceActiveRunways(newDep, newArr);
            m_activeRunwayMutex.clear();
            createActiveRunwayMutexes();
        }
        ...
        {
            auto found = m_activeRunwayMutex.find(runwayName);
            if (found != m_activeRunwayMutex.end())
            {
                return found->second;
            }

            auto runway = airport()->tryFindRunway(runwayName);
            if (runway)
            {
                const string keys[] = {
                    runway->name(),
                ...
                {
                    auto canonical = m_activeRunwayMutex.find(key);
                    if (canonical != m_activeRunwayMutex.end())
                    {
            ...
            throw runtime_error("Could not resolve runway mutex for runway [" + runwayName + "]");
        }
...
//                    event.type, intent->runwayName().c_str(), event.subject->callSign().c_str());
        ...
        void clearForTakeoff(const MutexEvent& event, const string& runwayName, uint64_t replyToId)
        {
            ...
            float initialHeading = GeoMath::addTurnToHeading(runwayEnd.heading(), m_departureInitialTurn);
            auto clearance = C.takeoffClearance(
                event.subject,
                initialHeading,
                event.immediate
        ...
        void clearToLand(const MutexEvent& event, const string& runwayName, uint64_t replyToId)
        {
            ...
            try {
                auto ground = airport()->groundAt(runwayEnd.centerlinePoint().geo());
                groundKhz = ground->frequency()->khz();
            ...
            {
                host()->writeLog(
                    "AICONT|TWR WARNING: unable to create landing clearance for flight[%s] runway[%s]",
