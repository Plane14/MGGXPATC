...
namespace
{
    ...
    shared_ptr<Airport> createDepartureOnlyControlledAirport(shared_ptr<HostServices> host)
    {
        ...
        auto gate = shared_ptr<ParkingStand>(new ParkingStand(
            ...
            UniPoint::fromGeo(host, header.datum()),
            0.0f,
            "C"));
        auto tower = WorldBuilder::assembleAirportTower(host, header, airspace, positions);
    ...
    shared_ptr<Airport> createApproachOnlyControlledAirport(shared_ptr<HostServices> host)
    {
        Airport::Header header("LEPA", "Palma Arrival Test", GeoPoint(39.5517, 2.7388), 0);
        auto airspace = WorldBuilder::assembleSampleAirportControlZone(header);
...
TEST(DepartureRunwayNormalizationTest, takeoffClearanceFallsBackWhenLocalMissing)
{
    ...
    auto flight = testFlight.ptr;
    flight->setPhase(Flight::Phase::Departure);
    flight->plan()->setDepartureGate("A1");
    flight->plan()->setDepartureRunway("14L");

    ASSERT_TRUE(!!airport->tryFindParkingStand("A1"));
    ASSERT_EQ(flight->plan()->departureGate(), "A1");
...
TEST(DepartureRunwayNormalizationTest, pushAndStartApprovalEndsAtTaxiStart)
{
    auto host = TestHostServices::create();
    auto clearanceFactory = make_shared<ClearanceFactory>(host);
    host->services().use<ClearanceFactory>(clearanceFactory);
    ...
    auto approval = clearanceFactory->pushAndStartApproval(flight);
    ASSERT_TRUE(!!approval);
    ASSERT_TRUE(!!approval->taxiPath());
    ASSERT_FALSE(approval->pushbackPath().empty());

    EXPECT_EQ(
        approval->pushbackPath().back(),
        approval->taxiPath()->edges[0]->node1()->location().geo());
    EXPECT_EQ(approval->pushbackPath().size(), 3u);
}

TEST(DepartureRunwayNormalizationTest, landingClearanceDoesNotFallBackToApproachWhenLocalMissing)
{
    auto host = TestHostServices::create();
    auto clearanceFactory = make_shared<ClearanceFactory>(host);
    host->services().use<ClearanceFactory>(clearanceFactory);

    auto airport = createApproachOnlyControlledAirport(host);
    auto world = WorldBuilder::assembleSampleWorld(host, { airport });
    host->useWorld(world);
    ...
    flight->plan()->setArrivalRunway("06R");

    auto landingClearance = clearanceFactory->landingClearance(flight, "06R", 121700);
    EXPECT_EQ(landingClearance, nullptr);
}
