...
TEST(ManeuverFactoryTest, taxiStraight_acceleratesFromCurrentSpeed)
{
    ...
    aircraft->setLocation(GeoPoint(10, 40));
    aircraft->setGroundSpeedKt(0.0);
    flight->setPhase(Flight::Phase::Departure);

    auto maneuver = factory->taxiStraight(
        flight,
        GeoPoint(10, 40),
        GeoPoint(10, 41),
    ...
    const double firstSpeed = aircraft->groundSpeedKt();
    EXPECT_GT(firstSpeed, 0.0);
    EXPECT_LT(firstSpeed, 11.7);

    maneuver->progressTo(chrono::milliseconds(1300));
    EXPECT_GT(aircraft->groundSpeedKt(), firstSpeed);
    EXPECT_LT(aircraft->groundSpeedKt(), 11.7);
}

TEST(ManeuverFactoryTest, aircraftProgressDoesNotDoubleApplyDirectPositionManeuver)
{
    auto host = TestHostServices::createWithWorld();
    ...
    auto aircraft = dynamic_pointer_cast<AIAircraft>(flight->aircraft());
    flight->setPhase(Flight::Phase::Departure);
    aircraft->setLocation(GeoPoint(10.0, 40.0));
    aircraft->setGroundSpeedKt(120.0);
    aircraft->setAttitude(aircraft->attitude().withHeading(90.0));

    aircraft->setManeuver(shared_ptr<Maneuver>(new AnimationManeuver<GeoPoint>(
        "direct_position",
        GeoPoint(10.0, 40.0),
        GeoPoint(10.0, 41.0),
        chrono::seconds(10),
        ...
        [aircraft](const GeoPoint& value, double) {
            aircraft->setLocation(value);
        }
    )));

    aircraft->progressTo(chrono::seconds(0));
    aircraft->progressTo(chrono::seconds(5));

    EXPECT_NEAR(aircraft->location().latitude, 10.0, 1e-9);
    EXPECT_NEAR(aircraft->location().longitude, 40.5, 1e-9);
}

TEST(ManeuverFactoryTest, taxiStop_deceleratesSmoothly)
{
    auto host = TestHostServices::createWithWorld();
    shared_ptr<ManeuverFactory> factory(new ManeuverFactory(host));
    ...
    auto aircraft = dynamic_pointer_cast<AIAircraft>(flight->aircraft());
    aircraft->setLocation(GeoPoint(10, 40));
    aircraft->setGroundSpeedKt(20.0);

    auto maneuver = factory->taxiStop(flight);
    EXPECT_TRUE(maneuver->isProxy());

    maneuver->progressTo(chrono::milliseconds(1000));
    EXPECT_NEAR(aircraft->groundSpeedKt(), 20.0, 0.01);

    maneuver->progressTo(chrono::milliseconds(2000));
    EXPECT_GT(aircraft->groundSpeedKt(), 0.0);
    EXPECT_LT(aircraft->groundSpeedKt(), 20.0);

    maneuver->progressTo(chrono::milliseconds(4000));
    EXPECT_NEAR(aircraft->groundSpeedKt(), 0.0, 0.1);
}
...
TEST(ManeuverFactoryTest, obstacleScanSemaphore_detectsHeadOnTraffic)
{
    auto host = TestHostServices::createWithWorld();

    auto ourFlight = createTestFlight(host);
    auto otherFlight = createTestFlight(host);
    host->getWorld()->addFlight(ourFlight);
    host->getWorld()->addFlight(otherFlight);

    auto ourAircraft = dynamic_pointer_cast<AIAircraft>(ourFlight->aircraft());
    auto otherAircraft = dynamic_pointer_cast<AIAircraft>(otherFlight->aircraft());

    ourFlight->setPhase(Flight::Phase::Departure);
    otherFlight->setPhase(Flight::Phase::Departure);

    ourAircraft->setLocation({ 10.0000, 40.0000 });
    ourAircraft->setAttitude({ 0.0, 0.0, 0.0 });
    otherAircraft->setLocation({ 10.0002, 40.0000 });
    otherAircraft->setAttitude({ 180.0, 0.0, 0.0 });

    auto state = ManeuverFactory::obstacleScanSemaphore(
        host->getWorld(),
        ourFlight,
        false,
    ...
    EXPECT_EQ(state, Maneuver::SemaphoreState::Closed);
}

TEST(ManeuverFactoryTest, obstacleScanSemaphore_arrivalTreatsDepartureAheadAsObstacle)
{
    auto host = TestHostServices::createWithWorld();
    ...
    ourFlight->setPhase(Flight::Phase::Arrival);
    otherFlight->setPhase(Flight::Phase::Departure);

    ourAircraft->setLocation({ 10.0000, 40.0000 });
    ourAircraft->setAttitude({ 0.0, 0.0, 0.0 });
    otherAircraft->setLocation({ 10.0002, 40.0000 });
    otherAircraft->setAttitude({ 0.0, 0.0, 0.0 });
...
TEST(ManeuverFactoryTest, obstacleScanSemaphore_pushbackTimeoutEventuallyOpens)
{
    auto host = TestHostServices::createWithWorld();
    ...
    ourFlight->setPhase(Flight::Phase::TurnAround);
    otherFlight->setPhase(Flight::Phase::TurnAround);
    ...
    auto pushbackState = ManeuverFactory::obstacleScanSemaphore(
        host->getWorld(),
        ourFlight,
        true,
        Maneuver::SemaphoreState::Closed,
        chrono::seconds(46));
    auto taxiState = ManeuverFactory::obstacleScanSemaphore(
        host->getWorld(),
        ourFlight,
        false,
        Maneuver::SemaphoreState::Closed,
    ...
    EXPECT_EQ(pushbackState, Maneuver::SemaphoreState::Open);
    EXPECT_EQ(taxiState, Maneuver::SemaphoreState::Closed);
}
